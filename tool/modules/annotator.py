...
class Annotator:
    ...
    def update_limits(self):
        ...
        for i in range(self.loop_counter):
            self.lower_limit = np.append(self.lower_limit,[0,1])
            self.upper_limit = np.append(self.upper_limit,[64,32])
        for i in range(self.array_counter):
            self.lower_limit = np.append(self.lower_limit,[0,0])
            self.upper_limit = np.append(self.upper_limit,[6,1])  
