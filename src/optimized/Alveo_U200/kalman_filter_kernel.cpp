...
#pragma HLS array_partition variable=in_local cyclic factor=32 dim=1
#pragma HLS array_partition variable=out_local cyclic factor=32 dim=1

		INTERFACE_WIDTH temp;
		int counter = 0;
READ_LOOP:	for(int i = 0; i < BATCH_SIZE_1 / FLOATS_PER_ELEMENT; i++) {
			#pragma HLS pipeline II=1
				temp = in[i];

READ_LOOP_01:	for(int j = 0; j < FLOATS_PER_ELEMENT; j++) {
					#pragma HLS pipeline II=1
...
INT_BATCH_INIT_LOOP:	for (int i = 0; i < INTERNAL_BATCHES; i++) {
			#pragma HLS pipeline II=1
			#pragma HLS unroll
...
KALMAN_CALC_LOOP:for (int t = 1; t < INTERNAL_BATCH_SIZE; t++) {

KALMAN_CALC_LOOP_01:	for (int i = 0; i < INTERNAL_BATCHES; i++) {
																	#pragma HLS unroll
		...
		counter = 0;
WRITE_LOOP:	for(int i = 0; i < BATCH_SIZE_1 / FLOATS_PER_ELEMENT; i++) {
				#pragma HLS pipeline II=1
				temp = 0;

WRITE_LOOP_01:	for(int j = 0; j < FLOATS_PER_ELEMENT; j++) {
					#pragma HLS pipeline II=1
					raw_float t;
