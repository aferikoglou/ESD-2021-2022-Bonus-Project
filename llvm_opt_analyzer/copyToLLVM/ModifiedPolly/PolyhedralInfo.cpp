...
bool PolyhedralInfo::isParallel(Loop *L) const {
  bool loopParallel = checkParallel(L);
  unsigned lloc = L->getStartLoc().getLine();
  char loopFile[512];
  strcpy(loopFile,L->getStartLoc()->getFilename().data());
  outs() << "^pollyAnalysis@@|" << "LoopLine:" << lloc << "|LoopFile:" << loopFile  << "|Parallel:"<< loopParallel << "\n";
  return loopParallel;
}
